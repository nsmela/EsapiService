...
namespace EsapiService.Generators.Contexts.ContextFactory {
    public class ComplexCollectionMethodFactory : IMemberContextFactory {
        public IEnumerable<IMemberContext> Create(ISymbol symbol, CompilationSettings settings) {
            ...
            // 3. Analyze Inner Type (Must be a Wrapped Type)
            ...
            if (innerType is not INamedTypeSymbol innerNamed || !settings.NamedTypes.IsContained(innerNamed)) {
                // If inner type is NOT wrapped (e.g. IEnumerable<string>), this factory yields nothing.
                // It will be picked up by SimpleCollectionMethodFactory later.
                yield break;
            }
            ...
            string callArgs = string.Join(", ", method.Parameters.Select(p => p.Name)); //TODO to replace BuildCallArgs

            // Generate the Interface and Wrapper names for the collection
            // e.g. "IReadOnlyList<IPlanSetup>"
            ...
            // e.g. "IReadOnlyList<AsyncPlanSetup>"
            ...
            // 5. Create Context
            yield return new ComplexCollectionMethodContext(
                Name: method.Name,
